...
export default function SendNotificationDashboard() {
 ...
 const callDashboardAPI = async (fDate, tDate) => {
  try {
    ...
    const response = await axios.get(`${BASE_URL}Campaign/Dashboard`, {
    ...
    const totals = resp.reduce(
      (acc, item) => {
        acc.MsgSentCnt += item?.MsgSentCnt ?? 0;
        acc.MsgFailedCnt += item?.MsgFailedCnt ?? 0;
         return acc;
      },
      {
        MsgSentCnt: 0,
...
//       const dateKey = dayjs(item.CreatedDate).format("YYYY-MM-DD");
...
//       groupedData[dateKey].success += item.MsgSentCnt ?? 0;
//       groupedData[dateKey].failure += item.MsgFailedCnt ?? 0;
//     });
...
//     const successData = sortedDates.map(
//       (date) => groupedData[date].success
//     );

//     const failureData = sortedDates.map(
//       (date) => groupedData[date].failure
//     );
...
const fetchChartData = async (ChartfromDate, CharttoDate) => {
  try {
    ...
    apiData.forEach((item) => {
      ...
      const formattedDate = dayjs(item.CreatedDate).format("YYYY-MM-DD");
      const index = dates.indexOf(formattedDate);

      if (index !== -1) {
        successData[index] += item.MsgSentCnt ?? 0;
        failureData[index] += item.MsgFailedCnt ?? 0;
      }
    });
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Paper sx={cardStyle} elevation={1}>
              ...
              <TextBox>
                <label>Total Success Contact</label>
                <h2>
                  <CountUp
                    end={counts.MsgSentCnt}
                    duration={2}
                    separator=","
                  />
                </h2>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Paper sx={cardStyle} elevation={1}>
              ...
              <TextBox>
                <label>Total Failed Contact</label>
                <h2>
                  <CountUp
                    end={counts.MsgFailedCnt}
                    duration={2}
                    separator=","
                  />
                </h2>
            ...
            <Paper elevation={1} sx={{ borderRadius: 3, py: 3 }}>
              ...
              >
                ...
                >
                  ...
                  >
                    Day-Wise Contact Success vs Failure 
                  </Typography>
                </Grid>
